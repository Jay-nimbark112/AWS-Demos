...
A VPC is a logically isolated network within the AWS cloud where you can launch AWS resources (like EC2 instances, RDS databases, etc.).

You have full control over network configuration, including:
...
1. **Q: Difference between Security Groups and ACLs in a VPC?**
    ...
    - **Ans:** A Security Group defines which traffic is allowed TO or FROM an EC2 instance. Whereas ACL controls at the SUBNET level, inspect the traffic TO or FROM a Subnet.

1. **Q: Can an EC2 instance in a VPC establish a connection with the internet?**
...
1. **Q: What is AMI?**
    ...
    - **Ans:** AMI is defined as Amazon Machine Image. Basically it’s a template comprising a software configuration part. For example, Operating System, DB Server, Application Server, etc,,,
...
1. **Q: When you launch a standby Relational Database Service instance, will it be available in the same Available Zone?**
    ...
    - **Ans:** Not advisable. The purpose of having a standby RDS  instance is to avoid an infrastructure failure. So you have to keep your standby RDS service in a different Availability Zone, which may have different infrastructure.
...
AWS Lambda is a serverless compute service provided by Amazon Web Services that lets you run code without managing servers.

You simply upload your code, and Lambda runs it automatically in response to events.
 In simple words: AWS Lambda lets you run code without creating or managing a server.
...
1. **Q: What is the use of Route Table?**
    ...
    - **Ans:** Route Table is used to route the network packets. Generally one route table would be available in each subnet. Route table can have any no. of records or information, hence attaching multiple subnets to a route table is also possible.
...
Temporary (ephemeral) storage attached directly to the EC2 host

Data is lost if the instance is stopped or terminated

Very high performance (physically attached disk)

Cannot be detached and attached to another instance

Best for: Cache, buffers, temporary files
...
Data remains even if instance is stopped

Can be detached and attached to another instance

Supports snapshots and backups

Best for: Databases, OS, long-term storage

1. **Q: EBS can be attached to any running instance that is in the same Availability Zone ?**
...
1. **Q: What is the difference between Stack and Template in Cloud Formation ?**
    ...
    - **Ans:**🔹 What is a Template?

A Template is a blueprint (JSON or YAML file).

It defines the infrastructure you want to create.
...
It describes what resources should be created.

👉 Think of it as an architectural design.

🔹 What is a Stack?

A Stack is the actual deployed instance of a template.

When you run a template in CloudFormation, it creates a stack.

The stack contains all the AWS resources defined in the template.

You can update, delete, or manage resources through the stack.

👉 Think of it as the finished building created from the blueprint.
